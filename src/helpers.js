...
exports.memoryFilter = function(docs, requestDef) {
    ...
    return filterInMemoryFields(rows, requestDef, userFields).map(function(
        row
    ) {
        return row.doc;
    });
...
exports.createFieldSorter = function(sort) {
    ...
    var directions = sort.map(function(sorting) {
        if (typeof sorting !== 'string' && getValue(sorting) === 'desc') {
            return -1;
        }
        return 1;
    });
...
//remove getFieldFromDoc and just import it from pouchdb-selector-core
